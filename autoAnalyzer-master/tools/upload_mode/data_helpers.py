...
import os
import copy
#from access_redis import Redis_data
count = 0
...
def get_cr_list_from_redis_data(self):
    Redis = Redis_data()
    cr_list = Redis.read_data()
    return cr_list

def generate_category_vec():
    train_cr_order = "./runs/train_cr_order.txt"
    f_handle = open(train_cr_order,'r')
    data = f_handle.read()
    cr_list = data.split("\n")
    f_handle.close()
    #cr_list = get_cr_list_from_redis_data()
    cr_list_dic = {}
    count = 0
    lable_flag = []
...
def strore_train_cr_categary_order_to_redis_database(cr_list):
    train_cr_order = "./train_cr_order.txt"
    f_handle = open(train_cr_order,'r')
    data = f_handle.read()
    cr_list = data.split("\n")
...
def load_journal_and_labels_train(data_file_path,dev_sample_percentage_rate):
    ...
    #strore_train_cr_categary_order_to_redis_database(cr_list)
    for cr in cr_list:
